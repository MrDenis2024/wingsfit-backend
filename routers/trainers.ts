...
trainersRouter.get("/", auth, async (req: RequestWithUser, res, next) => {
  try {
    if (req.user?.role === "client") {
      const findClient = await Client.findOne({ user: req.user?._id });
      ...
      return res.send(matchingTrainers);
    }
...
trainersRouter.post(
  ...
  async (req: RequestWithUser, res, next) => {
    try {
      ...
      await trainer.populate(
        "user",
        "_id email firstName lastName role phoneNumber gender timeZone dateOfBirth avatar",
      );
