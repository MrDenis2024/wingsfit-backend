...
chatsRouter.get("/groupChats", auth, async (req: RequestWithUser, res, next) => {
  try {
    const user = req.user;
    ...
    const userGroups = await Group.find({
      $or: [
        ...
        {course: {$in: await Course.find({user: user._id}).distinct("_id")}},
      ],
    });
...
chatsRouter.get("/privateChats", auth, async (req: RequestWithUser, res, next) => {
  try {
    const user = req.user;
    ...
    const privateChat = privateChats.length > 0
