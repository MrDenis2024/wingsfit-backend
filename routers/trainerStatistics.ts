...
trainerStatisticsRouter.get("/groups",auth ,async (req: RequestWithUser, res, next) => {
    const userId = req.user?._id;

    const courses = await Course.find({ user: userId });

    const coursesWithGroups = await Promise.all(courses.map(async (course) => {
            const groups = await Group.find({ course: course._id }).populate("clients", "firstName lastName phoneNumber");
...
trainerStatisticsRouter.get("/clients", auth, async (req: RequestWithUser, res, next) => {
    try {
        const userId = req.user?._id;
