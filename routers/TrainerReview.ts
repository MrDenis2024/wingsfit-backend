...
trainerReviewRouter.post("/", auth ,async (req: RequestWithUser, res, next) => {
    ...
    if (isEmpty(trainerId) || !rating || rating < 1 || rating > 5 || isEmpty(comment)) {
        return res.status(400).send({ error: 'Make sure that all fields are filled in correctly.' });
    }
    ...
    try {
        ...
        if (!isTrainer || isTrainer.role !== "trainer") {
            return res.status(404).send({ error: 'Trainer ID is wrong or user is not a trainer' });
        }
        ...
        if (!lessons || lessons.length === 0) {
            return res.status(404).send({ error: 'No lessons found for this course.' });
        }
        ...
        if (!isParticipant) {
            return res.status(403).send({ error: 'You must have attended a session with this trainer to leave a review.' });
        }
...
trainerReviewRouter.delete("/:id" , auth , async (req: RequestWithUser ,res,next) => {
    ...
    try{
        ...
        if (String(review.clientId) !== String(clientId) && req.user?.role !== "admin") {
            return res.status(403).send({ error: 'You do not have permission to delete this review.' });
        }
